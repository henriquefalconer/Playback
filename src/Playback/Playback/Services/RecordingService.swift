...
final class RecordingService: ObservableObject {
    ...
    private func captureScreen() async -> Data? {
        do {
            ...
            // Convert CGImage to PNG via ImageIO. Do NOT route through
            // NSImage/tiffRepresentation/NSBitmapImageRep — that path leaks
            // ~5 MB per capture (one full-screen bitmap every 2s).
