...
### 1.1 Menu Bar: "Open Timeline" Does Not Launch App
- **Spec:** `specs/menu-bar.md` lines 25-30 — "Open Timeline" should launch Playback.app if not running
- **Source:** `MenuBarViewModel.swift:85-90`
- **Problem:** Only searches existing `NSApp.windows` — if timeline window is not already open, nothing happens
- **Fix:** Use `NSWorkspace.shared.open()` or `WindowGroup` scene management to open/create the timeline window
- **Same issue affects:** "Settings..." button (`MenuBarViewModel.swift:92-97`) and "Diagnostics" button

### 1.2 Menu Bar: Record Screen Toggle Broken Without Permissions
...
### 1.3 Timeline: No Error States or Empty States
...
### 1.4 Timeline: No Loading Screen During Processing
...
### 1.6 65 Debug Print Statements in Production Code
